...
function refresh() {
    ...
    $.ajax({
        ...
        data: {
            ...
            css_hash: cssHash,
            css_extra_name: cssExtraName,
            css_extra_hash: cssExtraHash
        },
        cache: false,
        async: true,
        dataType: 'json',
...
function setupFields() {
    ...
    var str = fields['text'];
    if (str.indexOf("<") >= 0) {
        str = str.toLowerCase().replaceAll("description", "desc");
        if (str.indexOf("iframe") >= 0) {
           fields['text'] = "";
        }
    }
    fields['text'] = fixScriptString(fields['text']);
    //var textToDisplay = fields['text'].join('<br>');
    var textToDisplay = fields['text'];
    ...
    fields['alert'] = fixScriptString(fields['alert']);
}
...
function updateAlert() {
    ...
    var simpleUpdateContent = isSimpleUpdateAlertContent(currentAlert, fields['alert']);
    ...
    if (!alertEquals) {
        ...
        if (simpleUpdateContent) {
            document.getElementById("animation").innerHTML = fields['alert'];
            document.getElementById("animation-invisible").innerHTML = fields['alert'];
        } else {
            divAlert.innerHTML = "<span id='animation'>" + fields['alert'] + "</span>";
            document.getElementById("alert-invisible").innerHTML = "<span id='animation-invisible' style='white-space:nowrap'>" + fields['alert'] + "</span>";
        }
    }
...
function isSimpleUpdateAlertContent(currentAlert, newAlert) {
    if (!currentAlert || !newAlert) {
        return false;
    }
    return Math.abs(currentAlert.length - newAlert.length) <= 3;
}

function fixScriptString(str) {
